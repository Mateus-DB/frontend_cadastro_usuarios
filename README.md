...
A aplicação permite cadastrar usuários através de um formulário.

Os dados são enviados para o banco de dados através da rota POST da API.

Os usuários cadastrados são exibidos dinamicamente na interface através da rota GET.

Também é possível remover usuários diretamente pela interface utilizando a rota DELETE da API.
